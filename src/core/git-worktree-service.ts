...
export class GitWorktreeService {
	...
	async mergeWorktree(worktreePath: string, targetBranch: string): Promise<MergeResult> {
		try {
			...
			const mergeResult = await this.executeGitCommand([
				"merge",
				"--no-ff",
				currentBranch,
				"-m",
				`Merge branch '${currentBranch}' into ${targetBranch}`,
			]);
		...
		try {
			...
			if (!result.success) {
				...
				if (result.error.includes("rejected")) {
					...
					if (result.error.includes("fetch first")) {
						return {
							success: false,
	...
	async pullWorktree(
		worktreePath: string,
	): Promise<{ success: boolean; message: string; conflicts?: string[]; suggestions?: string[] }> {
		try {
			...
			if (!pullResult.success) {
				...
				if (pullResult.error.includes("divergent branches")) {
					return {
						success: false,
